...
- 来源优先级：X 重点账号最高、X 全网关键词其次，其余来源再按综合热度排序。
...
- 第 1 组：重点账号 1-15，最多 20 条。
- 第 2 组：重点账号 16-30，最多 20 条。
- 第 3 组：全网强相关关键词补充，最多 10 条。

重点账号不足时可以分页补齐，但总量不会超过 50 条，也不会为了凑数加入不相关内容。查询排除纯转发和回复，按 relevancy 获取最近 7 天内的结果。严格关键词为 AI、LLM、GPT、Claude、Gemini、DeepSeek、Qwen、Cursor、Agent、MCP、Model、Coding、大模型、智能体、开源模型。所有 X 请求/资源计数写入 `source_cursors`，用于节流和成本估算。

聚合结果先按来源优先级分层，再在同层内按热度排序：X 重点账号、X 全网关键词、其他免费来源。X 每次最多展示 50 条，其余每个来源分别最多展示 30 条。
