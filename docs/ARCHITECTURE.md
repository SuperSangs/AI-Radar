...
`x-ai` 使用 X Recent Search，默认 `X_MAX_RESULTS=70`、每天最多 1 次、两次请求至少间隔 24 小时。一次采集最多消耗 70 个 tweet resources：

- 第 1 组：官方账号，最多 15 条。
- 第 2、3 组：重点人物两组，分别最多 12 和 13 条。
- 第 4 组：带链接的严格 AI 关键词观点发现，最多 30 条。

重点账号不足时可以分页补齐，但总量不会超过 50 条，也不会为了凑数加入不相关内容。查询排除纯转发和回复，显式限定最近 24 小时；官方按 recency，其余按 relevancy 获取结果。严格关键词为 AI、LLM、GPT、Claude、Gemini、DeepSeek、Qwen、Cursor、Agent、MCP、Model、Coding、大模型、智能体、开源模型。采集器同时请求 `article`、`note_tweet` 和 `public_metrics`，将 X Article、长帖与达到传播阈值的观点帖归入“讨论”，并保留浏览、点赞、转发和收藏等指标供两端展示。它不会展开作者用户对象，避免额外 User Read 费用。所有 X 请求/资源计数写入 `source_cursors`，用于节流和成本估算。
