...
- (NSString *)panelHTML {
    ...
    "function updateViewStatus(){var el=document.getElementById('status');if(statusError)return;if(activeFilter==='discussion')el.textContent='近 3 天高热观点 · '+discussionItems.filter(matches).length+'条';else if(activeFilter==='paper')el.textContent='近 7 天社区热门论文 · '+paperItems.filter(matches).length+'条';else if(recentCount!==null)el.textContent='最近 24 小时 · '+recentCount+'条'}"
