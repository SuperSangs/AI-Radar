...
- X 热帖采用官方、重点人物、研究评测和技术话题搜索，默认每 2 小时增量补采，以最近 24 小时筛选去重后 30 条为目标；每日付费读取硬上限 70 条。
...
X_VISIBLE_TARGET=30
X_CYCLES_PER_DAY=12
X_CYCLE_INTERVAL_MINUTES=120
# 直连 api.x.com 不通时再填写：
# X_HTTPS_PROXY=http://127.0.0.1:7890
```

默认每页读取 10 条，按五路搜索的独立窗口与分页进度补采。未达到 30 条可见目标时每轮最多读取 60 条，达标后每轮最多读取 10 条；每天（UTC）总计不超过 70 条。不会要求帖子必须带链接，也不会用重复、过期或低质量内容凑数。请求 X Article、长帖正文和公开互动数据，但不展开作者用户对象。界面单独显示 X 实际条数，API 返回 `x_collection` 中的目标、可见条数和剩余额度。预算不足时如实显示缺口。详见 [X 增量采集策略](docs/X_INCREMENTAL_COLLECTION.md)。
